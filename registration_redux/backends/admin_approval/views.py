...
class RegistrationView(BaseRegistrationView):
    ...
    ``registration_redux.backends.default.views.RegistrationView`` but uses
...
class ActivationView(BaseActivationView):
    ...
    """
    Follows the exact logic of
    ``registration_redux.backends.default.views.ActivationView`` but uses
    ``SupervisedRegistrationProfile`` instead of ``RegistrationProfile``
...
class ResendActivationView(BaseResendActivationView):
    ...
    """
    Follows the exact logic of
    ``registration_redux.backends.default.views.ResendActivationView`` but uses
    ``SupervisedRegistrationProfile`` instead of ``RegistrationProfile``
...
class ApprovalView(BaseApprovalView):
    def approve(self, *args, **kwargs):
        ...
        ``registration_redux.signals.user_approved`` will be sent, with the

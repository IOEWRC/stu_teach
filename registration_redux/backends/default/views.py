...
class RegistrationView(BaseRegistrationView):
    """
    A registration_redux backend which follows a simple workflow:
    ...
    * ``registration_redux`` be listed in the ``INSTALLED_APPS`` setting
      (since this backend makes use of models defined in this
      application).

    * The setting ``ACCOUNT_ACTIVATION_DAYS`` be supplied, specifying
      (as an integer) the number of days from registration_redux during
      which a user may activate their account (after that period
      expires, activation will be disallowed).

    * The creation of the templates
      ``registration_redux/activation_email_subject.txt`` and
      ``registration_redux/activation_email.txt``, which will be used for
    ...
    Additionally, registration_redux can be temporarily closed by adding the
    setting ``REGISTRATION_OPEN`` and setting it to
    ``False``. Omitting this setting, or setting it to ``True``, will
    be interpreted as meaning that registration_redux is currently open and
    permitted.

    Internally, this is accomplished via storing an activation key in
    an instance of ``registration_redux.models.RegistrationProfile``. See
    ...
    def register(self, form):
        ...
        ``registration_redux.models.RegistrationProfile`` will be created,
        ...
        ``registration_redux.signals.user_registered`` will be sent, with
    ...
    def registration_allowed(self):
        """
        Indicate whether account registration_redux is currently permitted,
        based on the value of the setting ``REGISTRATION_OPEN``. This
        is determined as follows:

        * If ``REGISTRATION_OPEN`` is not specified in settings, or is
          set to ``True``, registration_redux is permitted.

        * If ``REGISTRATION_OPEN`` is both specified and set to
          ``False``, registration_redux is not permitted.
...
class ActivationView(BaseActivationView):
    ...
    def activate(self, *args, **kwargs):
        ...
        ``registration_redux.signals.user_activated`` will be sent, with the
...
class ResendActivationView(BaseResendActivationView):
    ...
    def render_form_submitted_template(self, form):
        ...
        return render(self.request,
                      'registration_redux/resend_activation_complete.html',
                      context)
